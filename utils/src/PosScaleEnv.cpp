...
PosScaleEnv::PosScaleEnv(pt start, pt goal, 
                         std::vector<std::string>& joints, 
                         const char* scene_name, const char* path, pt lows, pt highs, double pos_scale, double delta) {
    start_ = start;
    goal_ = goal;
...
void PosScaleEnv::setup_scene(std::vector<std::string>& joints, const char* scene_name, const char* path) {
    sg_ = aa_rx_sg_create();
    ...
    for (int i = 0; i < config_->len; i++) {
        config_->data[ids_[i]] = (scale_point(start_))[i];
...
pt PosScaleEnv::scale_point(pt point) {
    pt new_point = {point[0]*pos_scale_, point[1]*pos_scale_, point[2]*pos_scale_, point[3], point[4], point[5]};
    return new_point;
}

bool PosScaleEnv::outOfLimits(pt point) {
...
bool PosScaleEnv::outOfLimitsCollision(pt point) {
    for (int i = 0; i < N_; i++) {
...
bool PosScaleEnv::isStateValid(pt point) {
    if (delta_ > 0) {
    ...
    if (res == 0) {
        // saved2dPoints_ << "[";
        // for (int i = 0; i < 6; i++) {
        //     saved2dPoints_ << point[i];
        //     if (i != 5) saved2dPoints_ << ", ";
        // }
        // saved2dPoints_ << "], " << std::endl;
        return true;
    } else {
        return false;
    }
...
int PosScaleEnv::isStateValid_(pt point) {
...
double PosScaleEnv::penetrationDist(pt point, pt& close_point) {
    return 0;
}